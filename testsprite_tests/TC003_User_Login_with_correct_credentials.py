...
async def run_test():
    ...
    try:
        ...
        # -> Try refreshing the page to see if the chat interface loads properly or if any error messages appear
        frame = context.pages[-1]
        # Click on 'چیٹس' link to refresh the chat page and attempt to load chat interface
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Login Successful! Welcome to Chat Interface')).to_be_visible(timeout=30000)
        except AssertionError:
            raise AssertionError('Test case failed: User login was not successful, and the chat interface did not load as expected.')
